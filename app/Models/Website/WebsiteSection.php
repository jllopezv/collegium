...
class WebsiteSection extends Model
{
     ...
     */
    protected $fillable = [
        'title', 'priority', 'published', 'top', 'fixed', 'starred', 'description', 'body', 'image', 'website_section_cat_id'
    ];
    ...
    {
        return $this->belongsTo(WebsiteSectionCat::class,'website_section_cat_id','id');
    }
    ...
    public function getSectionImageAttribute()
    {
        ...
        if ( !Storage::disk('public')->exists( 'thumbs/'.$this->image ) || $showthumb==false)
        {
            ...
            {
                return Storage::disk('public')->url(config('lopsoft.sections_default_image'));
            }
            else
            {
