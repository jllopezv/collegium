        ...
        <x-lopsoft.datatable.mainlayout
            ...
            <x-slot name="header">
                <x-lopsoft.datatable.header-tr>
                    ...
                    <x-lopsoft.datatable.header-th  sortable sortorder='{{ $sortorder }}' columnname='language'>{{ strtoupper(__('lopsoft.language')) }}</x-lopsoft.datatable.header-th>
                    <x-lopsoft.datatable.header-th class='w-60' sortable sortorder='{{ $sortorder }}' columnname='code'>{{ strtoupper(__('lopsoft.code')) }}</x-lopsoft.datatable.header-th>
                    ...
                    @if($data->count())
                        @foreach($data as $index => $item)
                            <x-lopsoft.datatable.body-tr active='{{$item->active??true}}'>
                                ...
                                <x-lopsoft.datatable.row-column   link="{{ route($table.'.show',$item->id) }}">{{ $item->language }}</x-lopsoft.datatable.row-column>
                                <x-lopsoft.datatable.row-column  link="{{ route($table.'.show',$item->id) }}">{{ $item->code }}</x-lopsoft.datatable.row-column>
                                @if($showactions!='false')
