        ...
        <x-lopsoft.datatable.mainlayout
            ...
            <x-slot name="header">
                <x-lopsoft.datatable.header-tr>
                    ...
                    <x-lopsoft.datatable.header-th width='w-full' sortable sortorder='{{ $sortorder }}' columnname='name'>COLOR</x-lopsoft.datatable.header-th>
                    <x-lopsoft.datatable.header-th width='w-3/4'>VISTA PREVIA</x-lopsoft.datatable.header-th>
                    ...
                    @if($data->count())
                        @foreach($data as $index => $item)
                            <x-lopsoft.datatable.body-tr active='{{$item->active??null}}'>
                                ...
                                <x-lopsoft.datatable.row-column  link="{{ route($table.'.show',$item->id) }}">{{ $item->name }}</x-lopsoft.datatable.row-column>
                                <x-lopsoft.datatable.row-column  link="{{ route($table.'.show',$item->id) }}">{!! $item->tag !!}</x-lopsoft.datatable.row-column>
                                @if($showactions!='false')
